...
export const ShyftRewards = (): React.ReactElement => {
  ...
  const getRewardsAmount = async (address: string): Promise<void> => {
    ...
    const amount = await ShyftService.rewardsForAddress(address)
  ...
  const handledClaimClick = async (): Promise<void> => {
    ...
    if (rewardsAmount.gt(BigNumber.from(0))) {
      showLoadingToast(
        `Claiming ${Number(weiToShft(rewardsAmount.toString())).toPrecision(5)} SHFT from Shyft Pool.`,
        '/images/icons/shyft.svg'
      )
      await ShyftService.claimRewards({
        onError: () => {
          setShowLoader(false)
          Toast.dismiss()
        ...
        onSuccess: (txHash: string) => {
          ...
          Toast.dismiss()
          showSuccessToast(
            `Claimed ${Number(weiToShft(rewardsAmount.toString())).toPrecision(5)} SHFT from Shyft Pool.`
          )
          setShowLoader(false)
          setTimeout(() => {
            Toast.dismiss()
